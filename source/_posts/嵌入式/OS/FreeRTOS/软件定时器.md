...
停止先前使用 xTimerStart()、 xTimerReset()、 xTimerStartFromISR()、 xTimerResetFromISR()、 xTimerChangePeriod() 和 xTimerChangePeriodFromISR() API 函数创建的定时器。
...
xTimerStart()、 xTimerReset()、 xTimerStartFromISR()、 xTimerResetFromISR()、 xTimerChangePeriod() 和 [xTimerChangePeriodFromISR() 函数都可以用于将定时器转换为活跃状态。
