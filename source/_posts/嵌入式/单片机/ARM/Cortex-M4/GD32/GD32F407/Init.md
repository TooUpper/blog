...
下载相关文件，GD32F407 的用户手册、数组手册、选型手册等。
...
将官方标准固件库中我们需要的的文件拷贝到我们对应的文件目录中去
...
1. 将对应目录下文件都拷贝过去，然后将所有 .c 文件添加进项目中然后在解决错误问题（不建议）
2. 查看官方的示例代码，看看人家官方示例是需要拷贝那些，
   1. 打开 Keil 查看他的工程目录，然后从官方库中复制对应的文件到我们自己的项目中；(注意，一般工程中只会显示 .c 文件但我们要记得将对应的 .h 文件一起拷贝)
...
2. **运行他会报错，提示缺少一些文件，我们去看这些文件在我们的目录中是否存在，如果不存在就加进来**
...
此时，运行它使用的就是我们本地的 gd32f4xx_libopt.h 文件了，程序可以正常使用了；
