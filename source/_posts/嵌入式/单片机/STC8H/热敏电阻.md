...
查看原理图，了解热敏电阻使用的是哪一个引脚

**原理图**

![image-20240909163920080](C:\Users\kay\AppData\Roaming\Typora\typora-user-images\image-20240909163920080.png)
...
vcc 为 3.3 NTC 处电压为 ntc_V 那么经过 R20 处的电压就为 (3.3 - ntc_v) 那么流过 R20 的电流 I =  (3.3 - ntc_v) / 10
...
int NTC_GetTemperature() {
    ...
    // 根据电压求出电阻 电阻 = 电压 / 电流
    // 而串联电路电流处处相等下
    ntc_100R = ntc_V * 10 / ( 3.3 - ntc_V );
    
    // 用热敏电阻处的电阻与对照表中的数据进行比对进而得出温度值
    ntc_R = 10 * ntc_R；
    // 遍历数组，比较差值最小的，极为当前的温度
    // 默认第一个元素插值最小
    index = 0; 
...
‌**#ifndef在C语言中是“if not defined”的简写，属于宏定义的一种，用于条件编译。**‌
