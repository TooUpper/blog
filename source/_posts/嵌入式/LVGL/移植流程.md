...
拷贝官方 CST816T 的驱动程序等到我们工程中，因为驱动使用都是 I2C 进行通信所以这里还需要拷贝 I2C 的实现代码；
...
下载 [LVGL](https://github.com/lvgl/lvgl)（为了保证与 LVGL 图形化工具生成的代码兼容，二者最好选择一样的版本 v8.3）

lvgl 图形库下包含多个子目录，要使用 LVGL，只需要 **src** 目录下的 **.c** 和 **.h** 文件，以及 **lvgl/lvgl.h** 和 **lvgl/lv_version.h** 这两个文件即可。

**示例和演示**

**lvgl** 目录还包含了 **examples** 和 **demos** 目录。如果你的项目需要示例或演示，你可以将这些目录添加到你的项目中。如果使用 **make** 或 **CMake** 来处理 **examples** 和 **demos** 目录，那么就不需要额外的操作。
...
2. 在 Keil 中添加 lvgl 相关文件，这里要注意，我们并不是要拷贝 src 下的所有代码，他有些代码是为了兼容其他架构的我们就不需要添加进来;
...
3. src 目录下的代码能够生成一个 2D 的图像，但我们要将图像显示到屏幕上还需要一个驱动程序，所以这里我们还需要添加一份驱动程序的模板文件；
...
如果编译不报错，则代表初步移植完成，单此时他还会爆一些警告，因为我们还没有添加一些必须要的参数信息；
...
2. 屏幕显示驱动的代码一般是由硬件的供应商提供的，我们只需要引入到我们的工程目录下并在显示驱动文件中调用即可

3. 配置我们的输出缓冲区

LVGL 的工作模式是先将生成的图像存放在要给缓冲区中，然后通过硬件的显示驱动，将缓冲区中的数据显示到屏幕上，LVGL 提供了三种不同的缓冲区类型，这里我们需要配置选择一个
