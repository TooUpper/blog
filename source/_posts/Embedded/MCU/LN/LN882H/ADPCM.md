...
ADPCM 的出发点就是解决 DPCM 的差值宽度问题，通过定义一个差值表(例如 IMA ADPCM 中使用 89个固定差值，取值从7到32767)，将差值的范围放宽到 16bit，此时差值在数组中的编号只需要 6bit 就可以表示(0 - 88)，再进一步只记录编号的变化值，就将变化量压缩到了4bit。
...
- 差值步长码表: 下标从0到88，共89个值，从小到大，非均匀分布，下标越大，值之间的间隔越大。
...
**IMA ADPCM 格式说明**
例如一个 IMA ADPCM 编码值为 0x05, 对应二进制 0101, 其中最高位为0, 代表变化为正, 输出值是在前一个值上叠加； 低三位为 5, 代表差值步长下标变化量为 +4，也就是差值步长变大了, 另外三位的每一位分别代表对应实际差值的差值步长的倍数, 参与了差值的计算。
