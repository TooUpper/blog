...
新建 lvgl_drivers 目录，将 lvgl_drivers 下文件拷贝到 lvgl_drivers 目录下（LVGL v9 可以不用）：
![lvglmlgxyg](/public/image/Embedded/LVGL/lvglmlgxyg.png)

#### 编译器配置

准备完成后先**以管理员身份运行 VSCode**

通过命令面板 (Ctrl+Shift+P)，输入并选择 **CMake: Select a Kit**，选择我们要使用的 GCC 编译器；
